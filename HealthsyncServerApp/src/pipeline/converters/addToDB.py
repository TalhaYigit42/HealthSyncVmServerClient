...
def process_file(conn, file_path, override_user_id=None):
    ...
    if override_user_id is not None:
        for record in data:
            record['user_id'] = override_user_id
        
    if not isinstance(data, list) or not data:
        return 0

    REQUIRED = {
        "heart_rate_records": ["measured_at", "bpm"],
        "oxygen_saturation":  ["measured_at", "spo2_percent"],
        "sleep_sessions":     ["start_time", "total_sleep_minutes"],
        "step_records":       ["recorded_at", "steps"],
        "activities":         ["start_time", "activity_type"],
    }
    table_key = next((k for k in REQUIRED if filename.startswith(k)), None)
    if table_key:
        required_fields = REQUIRED[table_key]
        before = len(data)
        data = [d for d in data if all(d.get(f) is not None for f in required_fields)]
        dropped = before - len(data)
        if dropped:
            print(f"  ⚠ Dropped {dropped} record(s) with missing required fields in {os.path.basename(file_path)}")
    if not data:
        return 0
...
def main(output_dir, user_id=None):
    ...
    for filepath in sorted(json_files):
        try:
            records_count = process_file(conn, filepath, override_user_id=user_id)
