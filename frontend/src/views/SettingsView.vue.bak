    ...
    <main class="dashboard-main">
      ...
      <div class="settings-section">
        ...
        <div class="goals-grid">
          <div v-for="goal in healthGoals" :key="goal.label" class="goal-card">
            <div class="goal-header">
              <div :class="['goal-icon', goal.variant]">{{ goal.icon }}</div>
              <span class="goal-label">{{ goal.label }}</span>
            </div>
            <input type="text" :value="goal.value" readonly />
            <div class="goal-rec">{{ goal.rec }}</div>
          </div>
        </div>
      </div>
...
async function deleteAccount() {
  ...
  try {
    const token = localStorage.getItem('hs_token')
    // First verify password via login
...
const healthGoals = [
  { label: 'Daily Steps',      icon: '👟', value: '10.000', rec: 'Empfehlung: 10.000 Schritte', variant: 'steps' },
  { label: 'Sleep Hours',      icon: '🌙', value: '8h',     rec: 'Empfehlung: 7–9 Stunden',    variant: 'sleep' },
  { label: 'Calories to Burn', icon: '🔥', value: '500',   rec: 'Aktive Kalorien pro Tag',     variant: 'cal' },
  { label: 'Target Resting HR',icon: '❤️', value: '60',    rec: 'Gesund: 60–100 BPM',          variant: 'heart' },
]

const notifications = reactive([
  { name: 'Tägliche Erinnerungen',  desc: 'Werde erinnert, deine Aktivitäten zu loggen', on: true },
  { name: 'Achievement-Benachrichtigungen', desc: 'Benachrichtigungen wenn du Achievements freischaltest', on: true },
  { name: 'Wöchentliche Berichte',  desc: 'Erhalte wöchentliche Gesundheitszusammenfassungen', on: true },
...
.goals-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
...
.goal-card input { width: 100%; padding: 0.6rem 0.8rem; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 0.9rem; font-weight: 600; color: #0f172a; background: #fff; font-family: inherit; box-sizing: border-box; margin-bottom: 0.3rem; }
